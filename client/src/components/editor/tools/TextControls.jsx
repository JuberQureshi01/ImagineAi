...
import { Type, Trash2, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, AlignJustify } from "lucide-react";
import { useCanvas } from "@/context/CanvasContext";
import { IText } from "fabric";

const FONT_FAMILIES = [
  "Arial", "Arial Black", "Helvetica", "Times New Roman", "Courier New",
  "Georgia", "Verdana", "Comic Sans MS", "Impact", "Poppins", "Montserrat",
  "Roboto", "Lato", "Oswald", "Lobster"
];

// Increased the default font size from 20 to 48
const FONT_SIZES = { min: 8, max: 240, default: 48 };
...
export function TextControls() {
  ...
  useEffect(() => {
    ...
    updateSelectedText();
    const listeners = {
      "selection:created": updateSelectedText,
      "selection:updated": updateSelectedText,
      "selection:cleared": () => setSelectedText(null),
    };
    canvasEditor.on(listeners);
    return () => {
        if (canvasEditor) {
            canvasEditor.off(listeners);
        }
    };
  }, [canvasEditor]);
  ...
  const addText = () => {
    ...
    const text = new IText("Edit this text", {
      ...
      originX: "center",
      originY: "center",
      fontFamily: "Arial", // Start with a default font
      fontSize: FONT_SIZES.default, // Use the new, larger default size
      fill: "#000000",
      textAlign: 'left',
      editable: true,
      selectable: true,
  ...
  const toggleFormat = (format) => {
    // ... same as original, but using forceUpdate() at the end
    if (!selectedText) return;
     switch (format) {
      case "bold": {
    ...
    canvasEditor.requestRenderAll();
    forceUpdate(c => c + 1); // Force re-render to update button styles
  };
