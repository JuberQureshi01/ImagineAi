...
const FOCUS_MAP = { left: "fo-right", right: "fo-left", top: "fo-bottom", bottom: "fo-top" };
...
export function AIExtenderControls({ project }) {
  ...
  const hasBackgroundRemoval = () => getImageSrc(getMainImage())?.includes("e-bgremove");
 ...
 const calculateDimensions = () => {
  ...
  const selectDirection = (direction) => setSelectedDirection((prev) => (prev === direction ? null : direction));
  ...
  const applyExtension = async () => {
    ...
    try {
      ...
      const extendedImage = await FabricImage.fromURL(extendedUrl, { crossOrigin: "anonymous" });
