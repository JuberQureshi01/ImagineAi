...
import { Trash2, Palette, Image as ImageIcon, Search, Download, Loader2 } from "lucide-react";
...
const UNSPLASH_ACCESS_KEY = "i5D-UrMzsPNWgnBa51K0Kn5GqZXSG6DcL9WVbnIReY0";
...
export function BackgroundControls({ project }) {
  ...
  const handleBackgroundRemoval = async () => {
    ...
    try {
      ...
      const bgRemovedUrl = `${currentImageUrl.split("?")[0]}?tr=e-bgremove`;
      const processedImage = await FabricImage.fromURL(bgRemovedUrl, { crossOrigin: "anonymous" });
      
      const currentProps = {
        left: mainImage.left, top: mainImage.top, scaleX: mainImage.scaleX,
        scaleY: mainImage.scaleY, angle: mainImage.angle, originX: mainImage.originX,
      ...
      // Save the new state to the backend
      await api.patch(`/api/projects/${project.id}`, {
          currentImageUrl: bgRemovedUrl,
